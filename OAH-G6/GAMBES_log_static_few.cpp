...
//+PLUMEDOC BIAS GAMBESL
/*
*/
//+ENDPLUMEDOC

class GAMBESL : public Bias {
...
public:
  explicit GAMBESL(const ActionOptions&);
  void calculate();
  void prepare();
  static void registerKeywords(Keywords& keys);
};

PLUMED_REGISTER_ACTION(GAMBESL,"GAMBESL")

void GAMBESL::registerKeywords(Keywords& keys) {
  ...
  keys.addFlag("STATIC_BIAS_OPTIMIZE",false,"specifies the bias is static only in some basins");

}

GAMBESL::GAMBESL(const ActionOptions& ao):
  PLUMED_BIAS_INIT(ao),
  n_states_(0),
  n_gaussians_(0),
  resp(0),
...
{ 
  ...
  log.printf(" Reading Gaussian Mixture Model from files %s\n",std::string(filename));
  ...
  spline=!nospline;
  static_bias=false;
  static_bias_optimize=false;
  if(keywords.exists("STATIC_BIAS")){parseFlag("STATIC_BIAS",static_bias); }
  if(keywords.exists("STATIC_BIAS_OPTIMIZE")){parseFlag("STATIC_BIAS_OPTIMIZE",static_bias_optimize); }
  //if(static_bias){
  if(static_bias or static_bias_optimize){
...
void GAMBESL::prepare() {
  plumed.getAtoms().setCollectEnergy(true);
}

void GAMBESL::calculate()
{
  ...
  if(int(iter)<=int(pace_)) {bias=0.0;} 
  setBias(bias);
 ...
 // std::cout<<energy1<<" "<<bias<<std::endl;
  ...
  for(unsigned n=0; n<n_states_; n++) {
    ...
    visited[n]=1;
    double logz =std::log(std::exp(averages_k[n])+std::exp(beta*diff)); //add log(z1) instead of z1
    if(!isnan(logz)){averages_k[n]=logz;}
    //averages_k[n]+=std::exp(beta*diff);
    string s = std::to_string(n)+"_averages" ;
    getPntrToComponent(s)->set(averages_k[n]);
  }
...
double GAMBESL::update_bias(vector<double>& cv,vector<double>& cvder,vector<double>& u)
{ 
  double bias=0.0;
  unsigned int tot_gaussians=0;
  for(unsigned n=0; n<n_states_; n++) {
...
vector<double> GAMBESL::get_factor()
{ 
  vector<double> factor_c(n_states_,0.0);
  ...
  if(!static_bias){
    ...
    if(int(iter)==0){
     ...
     for(unsigned n=0; n<n_states_; n++) {
       expectation[n]=averages_k[n] ;
     }
   }
    for(unsigned n=1; n<n_states_; n++) { //state n=0 always has factor=1
      //factor_c[n]=expectation[0]/expectation[n];
      if((visited[n]==1)){ factor_c[n]=std::exp(expectation[0]-expectation[n]); }
      if(static_bias_optimize and static_factors[n]>=0){  factor_c[n]=static_factors[n];} // negative static-factor gets optimized 
     }
    }
  else{
     for(unsigned n=0; n<n_states_; n++) {
